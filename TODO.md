...
- [x] Monaco Editor 설치 (`@monaco-editor/react`, `monaco-editor`)
- [x] **Host(관리자) 에디터 진입** - 대시보드에서 프로젝트 클릭 시 에디터 화면으로 이동
- [x] 파일 트리 사이드바 UI 컴포넌트
- [x] Monaco Editor에 파일 내용 표시
- [ ] Express에서 Guest 앱 정적 파일 서빙
- [ ] JWT 토큰 인증 (승인된 유저만 에디터 접근)

### 2️⃣ 파일 시스템 연동 ✅ 완료!

- [x] `fs` 모듈로 프로젝트 폴더 구조 스캔
- [x] 파일/폴더를 계층 구조 JSON으로 변환
- [x] 파일 열기 API (클릭 시 내용 전송)
- [x] 파일 저장 API (Ctrl+S → Host 로컬 저장)
- [ ] Socket.io로 Guest에게 파일 트리 전송

### 3️⃣ Guest 에디터 (웹 브라우저)

- [ ] Guest용 React 앱 생성 빌드
- [ ] Express에서 정적 파일 서빙
- [ ] JWT 인증 미들웨어
- [ ] 로그인 성공 후 에디터 리다이렉트
